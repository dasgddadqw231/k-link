...
/**
 * 랜딩 카피 원칙 — 반드시 유지할 것.
 * 1. 건강 효능 광고는 사전 심의 대상이다(태국 식품법 제40·41조).
 *
 *    **2026-08-13 변경.** 이전 판은 "심의 전까지 어떤 기능성 표현도 넣지 말 것"
 *    이었다. 지금은 성분이 하는 일을 일반론으로 말하는 데까지 허용한다 —
 *    라이코펜은 지방과 함께일 때 흡수가 낫다, 베타카로틴은 몸에서 비타민A가
 *    된다 같은 교과서 수준의 진술이 그 범위다. 결정한 사람은 사용자다.
 *
 *    그래도 넘지 않는 선이 둘 있다.
 *    (a) 함량·퍼센트 같은 수치. 성분 분석 성적서가 오기 전에는 우리 제품의
 *        값을 모르므로 적으면 그냥 틀린 말이 된다.
 *    (b) 질병·체형·부종 — 치료·예방·다이어트·붓기. 태국에서 가장 강하게
 *        막힌 범주이고, 사실 여부와 무관하게 승인이 나지 않는다.
 *
 *    docs/serving.md에 무엇을 왜 넣고 뺐는지 줄 단위로 적어 뒀다.
...
export const t = {
  th: {
     ...
     *
     * 이 섹션 전체가 서 있는 사실 하나(그 문서 0-2절): 태국 소비자는 이미 아침에
     * 올리브유+레몬을 먹으려 하고 있고, 병을 사서 숟가락으로 계량하는 데서 막혀
     * 있다. 그래서 여기서는 가르치지 않는다 — 아는 사람에게 가르치면 무시당한다.
     * 제목·리드·맺음이 각각 그 사실의 어느 면을 잡는지는 바로 아래 주석들에 있다.
     *
     * 세 줄의 축은 세 제품에 똑같다: ① 그대로 ② 음식에 ③ 하나 더. 옆으로 비교되게 한다.
     *
     * 이 아홉 줄에는 효능을 넣지 않는다. 용법·용도·보관은 애초에 심의 대상이
     * 아니고(สรรพคุณ이 아니다), 여기에 "좋다"가 붙는 순간 아홉 줄 전체가 대상이
     * 된다. 효능은 위의 Why 한 줄이 따로 맡는다 — 그 줄에 허용된 범위는 파일
     * 상단 원칙 1에 적혀 있다.
     */
    serveLabel: "วิธีกิน",
    /*
     ...
     * 그 전에는 둘 다 없었는데, 없을 때 이 섹션은 설명서로 읽혔다 — 무엇을
     * 하라고만 하고 왜 먹는지도 어떤 맛인지도 말하지 않았다.
     *
     * Why는 성분이 하는 일을 일반론으로만 적는다(파일 상단 원칙 1 참고).
     * 수치를 안 쓰는 게 안전하기만 한 선택은 아니다 — 문헌상 아르베키나
     * 폴리페놀은 150~350 mg/kg로 EFSA 문턱(250 mg/kg) 위아래를 오가서 숫자를
     * 걸면 배치마다 틀릴 수 있고, 반대로 호박 착즙 카로티노이드는 문턱을 크게
     * 넘어 굳이 숫자가 없어도 손해가 없다.
     *
     * Taste의 근거는 품종이다. 아르베키나는 쓴맛이 거의 없고 아몬드로 끝나는
     * 스페인 품종이라, 여린 채소를 덮지 않는다는 말이 취향이 아니라 사실이 된다.
     */
    serveOlleWhy: "ไขมันดีจากน้ำมันมะกอก คู่กับความสดของเลมอน",
    serveOlleTaste: "อาร์เบกีนาจากสเปน นุ่ม ไม่ขม ทิ้งท้ายด้วยเลมอนสดชื่น",
    serveOltoWhy: "ไลโคปีนในมะเขือเทศดูดซึมได้ดีขึ้นเมื่อมีไขมัน เราจึงใส่มาในซองเดียวกัน",
    serveOltoTaste: "มะเขือเทศคั้นสดล้วน ๆ ไม่ข้น หมดซองสบาย ๆ",
    serveHobakWhy: "สีส้มของฟักทองแก่คือเบต้าแคโรทีน ที่ร่างกายเปลี่ยนเป็นวิตามินเอ",
  ...
  en: {
    ...
    serveOlleWhy: "The good fat of olive oil, with the freshness of lemon.",
    serveOlleTaste:
      "Spanish Arbequina — soft, no bitterness, a fresh lemon finish.",
    serveOltoWhy:
      "Lycopene absorbs better alongside fat, so the tomato arrives with its olive oil.",
    serveOltoTaste: "Pressed tomato and nothing else. Thin enough to finish.",
    serveHobakWhy:
      "That orange is beta-carotene, which the body turns into vitamin A.",
