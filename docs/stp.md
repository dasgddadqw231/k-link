...
- **태국의 목재포장재(ISPM 15) 검역 세부 요건** → 국제 기준과 IPPC 열처리 표시 자체는 확인했으나, 태국 적용 범위는 확인하지 못했다. 플라스틱 팔레트를 쓰면 이 문제 자체가 없어진다.
- **우리 거래의 인코텀즈·결제 조건·결제 통화**, 그리고 **적하보험을 누가 드는지** → 계약에서 정해야 할 것들인데 아직 정하지 않았다. 이게 비면 사고가 났을 때 비로소 다투게 된다.
